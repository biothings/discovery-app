(function () {
  ...
  extend(SwalForm.prototype, {
    ...
    generateHtmlForm: function () {
      var form = {
        ...
        innerHtml: this.formFields
          .map(toFormTag.bind(this))
      ...
      function toFormTag(field) {
        ...
        var conditionalLineBreak =
          input.isRadioOrCheckbox() && this.lastFieldName !== field.name
            ? "<br>"
            : "";
    ...
    getFormValues: function (isConfirm) {
      var inputHtmlCollection =
        document.getElementsByClassName("swal-form-field");
    ...
    allowClickingDirectlyOnInputs: function () {
      ...
      document.querySelector(".sweet-alert button.confirm").onblur =
        function () {};
      document.querySelector(".sweet-alert button.cancel").onblur =
        function () {};
    },
    ...
    addTabOrder: function () {
      var formFields = Array.prototype.slice.call(
        document.querySelectorAll(".swal-form .swal-form-field")
      );
  ...
  function Input(field) {
    var input = {
      ...
      toHtml: function () {
        ...
        if (input.type !== "select") {
          inputTag = t(
            "<input id='{id}' class='{clazz} swal-form-field' type='{type}' name='{name}'" +
              " value='{value}' title='{placeholder}' placeholder='{placeholder}'" +
              " data-swal-forms-required={required}>",
            input
          );
        } else {
          inputTag =
            t(
              "<select id='{id}' class='{clazz} swal-form-field' name='{name}'" +
                " value='{value}' title='{placeholder}' style='width:100%'>" +
                " data-swal-forms-required={}",
              input
            ) +
        ...
        function toHtmlOptions(optionsString, option) {
          ...
          return (
            optionsString +
            t("<option value='{value}'{selected}>{text}</option>", option)
          );
        }
      },
    };
    ...
    input.label =
      input.isRadioOrCheckbox() && input.label === ""
        ? input.value
        : input.label;
    input.clazz += input.isRadioOrCheckbox()
      ? " patch-swal-styles-for-inputs"
      : " nice-input";
