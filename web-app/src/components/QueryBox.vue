        ...
        <template v-for="(item, index) in q.domain">
          <template v-for="link in getLink(item)">
            <RouterLink
              ...
              :to="link"
              :class="[
                index !== q.domain.length - 1 ? 'text-muted' : textColor,
              ]"
            >
              <small>
          ...
          <li v-for="path in getPaths(q)">
            <span v-for="(item, index) in path">
              ...
              <span v-if="index !== path.length - 1"
                >&nbsp;<font-awesome-icon icon="fas fa-caret-right" :class="textColor"/>&nbsp;</span
              >
            </span>
          </li>
        </ul>
        ...
        <template v-for="(item, index) in q.range">
          <template v-for="link in getLink(item)">
            ...
            >
              ...
              <font-awesome-icon icon="fas fa-external-link-alt" v-if="link[0] !== '.'"/>
            </RouterLink>
          </template>
        </template>
      </template>
