...
def is_dde_metadata_owner(username):
    ...
    """
    r = requests.get(
        "https://discovery.biothings.io/api/dataset/query?facets=_meta.username&facet_size=1000"
    )
    if r.status_code == 200:
        res = r.json()
...
def change_ownership(_id=None, new_owner=None, dryrun=True):
    ...
    if current_owner == new_owner:
        logger.info(
            f"New owner: {new_owner} - Current owner: {current_owner} are the same. Terminating"
        )
        return
