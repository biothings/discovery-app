...
def main():
        ...
        index_name,
        {
            "settings": {
                ...
                "analysis": {
                    ...
                    "analyzer": {
                        "string_lowercase": {"filter": "lowercase", "tokenizer": "keyword"},
                        "whitespace_lowercase": {"filter": "lowercase", "tokenizer": "whitespace"},
                    },
                },
            },
            "mappings": {"properties": requests.get(MAPPING_URL).json(), "dynamic": False},
        },
    )
    logging.debug(_)
    ...
    _ = client.indices.update_aliases(
        {
            "actions": [
                {"remove": {"index": INDEX_PREFIX + "*", "alias": INDEX_ALIAS}},
                {
                    "add": {
                        "index": index_name,
                        "alias": INDEX_ALIAS,
