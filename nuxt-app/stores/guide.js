...
export const useGuideStore = defineStore("guideStore", {
  ...
  actions: {
    ...
    addBulkReport(payload) {
      for (let key in this.bulkReport) {
        if (this.bulkReport[key].includes(payload.value)) {
          var found = this.bulkReport[key].indexOf(payload.value);
          while (found !== -1) {
            this.bulkReport[key].splice(found, 1);
            found = this.bulkReport[key].indexOf(payload.value);
          }
        }
      }
      this.bulkReport[payload.field].push(payload.value);
      // console.log(this.bulkReport)
    },
    resetBulkReport() {
    ...
    toggleBeginBulkRegistration() {
      this.beginBulkRegistration = !this.beginBulkRegistration;
    },
    toggleDesc() {
      this.showDescriptions = !this.showDescriptions;
    },
    saveBulkItems(payload) {
      this.bulkJSONItems = payload["items"];
    },
    setUsePrefilled(payload) {
      this.useGuidePreFilled = payload;
    },
    updateJsonItem(payload) {
      let item = payload["item"];
      let change = payload["change"];
      let field = payload["field"];

      for (var i = 0; i < this.bulkJSONItems.length; i++) {
        if (
          this.bulkJSONItems[i]["name"] == item["name"] ||
          this.bulkJSONItems[i]["description"] == item["description"]
        ) {
          this.bulkJSONItems[i][field] = change;
    ...
    saveSchema(payload) {
      ...
      this.output["@type"] = this.schema?.name || this.schema?.label;
      this.output["@context"] = payload["schema"]["@context"];
      let obj = Object.assign({}, this.output);
      this.output_default = obj;

      // helper func to decide right subcategory
      function getProp(propname) {
        let cat = {
          category: this.schema.label || this.schema.name,
        };
        if (this.schema?.validation?.required?.includes(propname)) {
      ...
      for (var prop in this.schema.validation.properties) {
        if (
          this.schema.validation.properties[prop].hasOwnProperty("categories")
        ) {
          this.schema.validation.properties[prop]["categories"].unshift(
            getProp(prop)
          );
        } else {
          this.schema.validation.properties[prop]["categories"] = [];
          this.schema.validation.properties[prop]["categories"].unshift(
            getProp(prop)
          );
        }
    ...
    saveSchemaName(payload) {
      this.schemaName = payload["name"];
    },
    reset() {
      this.schema = {};
    ...
    markSelected(payload) {
      ...
      for (let key in this.schema.validation.properties) {
        if (key === selection) {
          this.schema.validation.properties[selection]["selected"] = true;
        } else {
          this.schema.validation.properties[key]["selected"] = false;
        }
      }
    },
    selectNext() {
      for (let key in this.schema.validation.properties) {
        if (
          this.schema.validation.properties[key] &&
          !this.schema.validation.properties[key].value
        ) {
          this.schema.validation.properties[key]["selected"] = true;
          break;
        }
      }
    },
    highlight(payload) {
      ...
      for (prop in this.schema.validation.properties) {
        this.schema.validation.properties[prop]["highlighted"] = false;
      }
      for (prop in this.schema.validation.properties) {
        if (
          this.schema.validation.properties[prop].hasOwnProperty("categories")
          ...
          for (
            var i = 0;
            i < this.schema.validation.properties[prop]["categories"].length;
            i++
          ) {
            if (
              this.schema.validation.properties[prop]["categories"][i][
                "category"
              ] === cat &&
              this.schema.validation.properties[prop]["categories"][i][
                "subcategory"
              ...
              if (this.schema.validation.properties[prop]["highlighted"]) {
                this.schema.validation.properties[prop]["highlighted"] = false;
              } else {
                this.schema.validation.properties[prop]["highlighted"] = true;
              }
            }
          }
        }
    ...
    markCompleted(payload) {
      ...
      if (this.schema.validation.properties.hasOwnProperty(selection.name)) {
        this.schema.validation.properties[selection.name]["value"] =
          selection.value;
        //prefill identifier with URL field if available
        if (
          selection.name == "url" &&
          "identifier" in this.schema.validation.properties &&
          !this.schema.validation.properties["identifier"]["value"] &&
          !this.schema.validation?.required?.includes("identifier")
        ) {
          this.schema.validation.properties["identifier"]["value"] =
            selection.value;
        }
        ...
        if (
          selection.name + "s" in this.schema.validation.properties &&
          this.schema.validation.properties[
            selection.name + "s"
          ...
          this.schema.validation.properties[selection.name + "s"]["value"] =
            selection.value;
        }
      }
    },
    clearValueOfProp(payload) {
      let selection = payload["clear"];
      if (
        this.schema.validation.properties[selection].hasOwnProperty("value")
      ) {
        delete this.schema.validation.properties[selection]["value"];
        this.schema.validation.properties[selection]["selected"] = false;
      } else {
        new Notify({
          status: "error",
          title: "Guide",
    ...
    removeArrayItemFrom(payload) {
      let field = payload["from"]; //name of prop
      let item = payload["item"]; //full info
      let props = this.schema.validation.properties;
    ...
    addToArrayFrom(payload) {
      let field = payload["from"];
      let item = payload["item"];
      //Initialize array
      if (!this.schema.validation.properties[field].value) {
        if (payload?.forceArray) {
          ...
          this.schema.validation.properties[field]["value"] = [item];
        } else {
          this.schema.validation.properties[field]["value"] = item;
        }
      } else {
        if (
          this.schema.validation.properties[field]["value"].constructor ===
          Object
        ) {
          this.schema.validation.properties[field]["value"] = [
            this.schema.validation.properties[field]["value"],
          ];
          this.schema.validation.properties[field]["value"].push(item);
        } else if (
          this.schema.validation.properties[field]["value"].constructor ===
          Array
        ) {
    ...
    formPreviewForGuide() {
      ...
      this.output = Object.assign({}, this.output_default);
      if (this.useGuidePreFilled) {
        for (var key in this.guide_prefilled) {
          this.output[key] = this.guide_prefilled[key];
        }
      }
          ...
          if (props[key]["value"] || props[key]["value"] === false) {
            if (
              ...
              if (
                ...
                this.output[key] = props[key].value;
              } else {
                //else take the one item and place out of the array
                this.output[key] = props[key].value[0];
              }
            } else {
              this.output[key] = props[key].value;
            }
      ...
      addFormats(ajv);
      var schema = this.schema.validation;
      ...
      if (!isValid) {
        ...
        for (var i = 0; i < this.errors.length; i++) {
          if (this.errors[i]["dataPath"]) {
            let parts = this.errors[i]["dataPath"].split("/");
            for (var x = 0; x < parts.length; x++) {
              let k = parts[x];
              if (k in props) {
                if ("hasIssue" in props[k]) {
                  props[k]["hasIssue"].push(this.errors[i]["message"]);
                } else {
                  props[k]["hasIssue"] = [this.errors[i]["message"]];
                }
              }
            }
          }
          if (this.errors[i]["params"].hasOwnProperty("missingProperty")) {
            let missing = this.errors[i]["params"]["missingProperty"];
            if (props.hasOwnProperty(missing)) {
              if ("hasIssue" in props[missing]) {
                props[missing]["hasIssue"].push(this.errors[i]["message"]);
              } else {
                props[missing]["hasIssue"] = [this.errors[i]["message"]];
              }
            }
          }
        }
    ...
    mergeCategoryProps(payload) {
      ...
      for (var i = 0; i < catpropsrequired.length; i++) {
        if (!this.schema.validation.required.includes(catpropsrequired[i])) {
          this.schema.validation.required.push(catpropsrequired[i]);
        }
      }
      ...
      for (prop in catprops) {
        if (prop in this.schema.validation.properties) {
        ...
        } else if (prop.slice(0, -1) in this.schema.validation.properties) {
          //plural form exists
          ...
          this.schema.validation.properties[prop] = newProp;
        } else {
          let newProp = catprops[prop];
          newProp["categories"] = [];
          // set origin to remove by origin
    ...
    removeCategory(payload) {
      ...
      for (var i = 0; i < this.selectedPortals.length; i++) {
        if (this.selectedPortals[i]["label"] === origin) {
          catpropsrequired = this.selectedPortals[i]["validation"]["required"];
        }
      }
      for (prop in this.schema.validation.properties) {
        // if origin matches remove prop
        if (
          this.schema.validation.properties[prop].hasOwnProperty("origin") &&
          this.schema.validation.properties[prop]["origin"] === origin
        ) {
          delete this.schema.validation.properties[prop];
        } else {
          ...
          if (
            ...
            for (
              ...
              if (
                ...
                this.schema.validation.properties[prop]["categories"].splice(
                  i,
                  1
                );
              }
    ...
    setStartingPoint(payload) {
      this.startingPoint = payload["startingPoint"];
    },
    addPortalSchema(payload) {
