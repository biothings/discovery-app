...
export const useEditorStore = defineStore("editorStore", {
  ...
  actions: {
    async getSchemaOrgVersion() {
      if (!this.schema_version) {
        let config = useRuntimeConfig();
        axios
          .get(config.public.apiUrl + "/api/schema/schema?meta=1")
          .then((res) => {
            if (res.data?._meta?.version) {
              this.saveSchemaVersion({ v: res.data?._meta?.version });
            } else {
    ...
    saveSchemaVersion(payload) {
      this.schema_version = payload.v;
      console.log(
        "%c Schema.org version: " + this.schema_version,
        ...
        if (Object.hasOwnProperty.call(schema, "validation")) {
          if (Object.hasOwnProperty.call(schema.validation, "properties")) {
            Object.keys(schema.validation["properties"]).forEach((propName) => {
              this.selectProp({ label: propName });
            });
          }
        }
      });
    ...
    async getParents() {
      ...
      let list = [];
      for (var i = 0; i < this.schema[0].parent_classes.length; i++) {
        list = list.concat(
          this.schema[0].parent_classes[i].split(", ").reverse()
        );
      }
      ...
      try {
        ...
        data.forEach((parentInfo) => {
          this.saveParent({ parent: parentInfo });
        });
    ...
    setRemoveValidation(payload) {
      this.removeValidation = payload.value;
    },
    toggleRemoveValidation() {
      this.removeValidation = !this.removeValidation;
      if (this.removeValidation) {
        new Notify({
          status: "warning",
          title: "Editor",
          text: "Validation OFF",
    ...
    saveSchemaForEditor(payload) {
      let names = this.schema.map((cls) => cls.name);
      // console.log(names)
      if (!names.includes(payload["schema"].name)) {
        console.log("✅ Adding Class: ", payload["schema"].name);
        this.schema.push(payload["schema"]);
        this.startingPoint = payload["start"];
        if (payload["schema"].hasOwnProperty("validation")) {
          //save on initial load as this.schema will change
          //later on validation will not be available.
          this.top_class_validation = payload["schema"].validation;
          ...
          if (cardinalityPresent) {
            this.addCardinality = true;
          }
        }
        console.log("✅ saved schema...", this.schema);
      } else {
    ...
    setValidation(payload) {
      ...
      this.finalschema["@graph"][0]["$validation"]["properties"][name] = newval;
    },
    resetValidationFor(payload) {
      let name = payload["name"];
      let obj = {};
      try {
        obj.description =
      ...
      this.finalschema["@graph"][0]["$validation"]["properties"][name] = obj;
      new Notify({
        status: "success",
        title: "Editor",
    ...
    savePrefix(payload) {
      ...
      this.finalschema["@context"][this.prefix] =
        "https://discovery.biothings.io/view/" + this.prefix + "/";
    },
    saveParent(payload) {
    ...
    addClass(payload) {
      ...
      newClass["name"] = this.prefix + ":" + payload["name"];
      newClass["prefix"] = this.prefix;
      newClass["parent_classes"] = [this.schema[0].name];
      console.log("🟠 Adding User Class: ", payload["name"]);
      this.schema.unshift(newClass);
    },
    addProperty(payload) {
      let newProp = {};
      newProp["label"] = payload["name"];
      newProp["curie"] = this.prefix + ":" + payload["name"];
      newProp["description"] = payload["description"];
      newProp["name"] = this.prefix + ":" + payload["name"];
      newProp["range"] = payload["range"];
      // domain id can already have prefix, if not add it.
      newProp["domain"] = payload["domain"].includes(":")
        ? payload["domain"]
        : this.prefix + ":" + payload["domain"];
      ...
      for (var i = 0; i < this.schema.length; i++) {
        if (this.schema[i].special) {
          if (Object.hasOwnProperty.call(this.schema[i], "properties")) {
            console.log(this.schema);
            //check if exists then update
            if (
              this.schema[i].properties.some(
                (prop) => prop?.label == payload["name"]
              )
            ) {
              for (var x = 0; x < this.schema[i].properties.length; x++) {
                if (this.schema[i].properties[x].label === newProp["label"]) {
                  // remove matching existing
                  this.schema[i].properties.splice(x, 1);
                  //replace with new
                  this.schema[i].properties.push(newProp);
                  new Notify({
                    status: "success",
                    title: "Editor",
            ...
            !Object.hasOwnProperty.call(this.schema[i], "properties")
          ) {
            this.schema[i]["properties"] = [];
            this.schema[i].properties.push(newProp);
          }
        }
      }
    ...
    selectProp(payload) {
      ...
      for (var i = 0; i < this.schema.length; i++) {
        if (Object.hasOwnProperty.call(this.schema[i], "properties")) {
          for (var x = 0; x < this.schema[i]["properties"].length; x++) {
            if (
              this.schema[i]["properties"][x].hasOwnProperty("label") &&
              this.schema[i]["properties"][x].label === label
            ) {
              if (this.schema[i]["properties"][x].hasOwnProperty("selected")) {
                this.schema[i]["properties"][x]["selected"] =
                  !this.schema[i]["properties"][x]["selected"];
                //If unselected also mark as not required
                if (!this.schema[i]["properties"][x]["selected"]) {
                  // reset all marginality
                  this.schema[i]["properties"][x]["isRequired"] = false;
                  this.schema[i]["properties"][x]["isOptional"] = false;
                  this.schema[i]["properties"][x]["isRecommended"] = false;
                }
              } else {
                this.schema[i]["properties"][x]["selected"] = true;
              }
            }
          }
        }
        ...
        if (Object.hasOwnProperty.call(this.schema[i], "properties")) {
          for (var x = 0; x < this.schema[i]["properties"].length; x++) {
            if (
              ...
              if (
                this.schema[i]["properties"][x].hasOwnProperty("isRequired")
              ) {
                this.schema[i]["properties"][x]["isRequired"] =
                  !this.schema[i]["properties"][x]["isRequired"];
              } else {
                this.schema[i]["properties"][x]["isRequired"] = true;
              }
              //reset other marginality
              if (
                this.schema[i]["properties"][x] &&
                this.schema[i]["properties"][x]["isRequired"]
              ) {
                this.schema[i]["properties"][x]["isOptional"] = false;
                this.schema[i]["properties"][x]["isRecommended"] = false;
        ...
        if (Object.hasOwnProperty.call(this.schema[i], "properties")) {
          for (var x = 0; x < this.schema[i]["properties"].length; x++) {
            if (
              ...
              if (
                this.schema[i]["properties"][x].hasOwnProperty("isOptional")
              ) {
                this.schema[i]["properties"][x]["isOptional"] =
                  !this.schema[i]["properties"][x]["isOptional"];
              } else {
                this.schema[i]["properties"][x]["isOptional"] = true;
              }
              //reset other marginality
              if (
                this.schema[i]["properties"][x] &&
                ...
                this.schema[i]["properties"][x]["isRequired"] = false;
                this.schema[i]["properties"][x]["isRecommneded"] = false;
              }
            }
          }
        }
        ...
        if (Object.hasOwnProperty.call(this.schema[i], "properties")) {
          for (var x = 0; x < this.schema[i]["properties"].length; x++) {
            if (
              ...
              if (
                this.schema[i]["properties"][x].hasOwnProperty("isRecommended")
              ) {
                this.schema[i]["properties"][x]["isRecommended"] =
                  !this.schema[i]["properties"][x]["isRecommended"];
              } else {
                this.schema[i]["properties"][x]["isRecommended"] = true;
              }
              //reset other marginality
              if (
                this.schema[i]["properties"][x] &&
    ...
    removeProperty(payload) {
      ...
      for (var i = 0; i < this.schema.length; i++) {
        if (this.schema[i].special) {
          ...
          for (var x = 0; x < this.schema[i].properties.length; x++) {
            if (this.schema[i].properties[x].label === label) {
              this.schema[i].properties.splice(x, 1);
              new Notify({
                status: "warning",
                title: "Editor",
    ...
    formPreview() {
      try {
        this.previousPreviewProps =
          this.finalschema["@graph"][0]["$validation"]["properties"];
      } catch (error) {
        console.log("NO previousPreviewProps", this.previousPreviewProps);
      }
      this.finalschema["@graph"] = [];
      ...
      for (var i = 0; i < this.schema.length; i++) {
        if (this.schema[i].special) {
          ...
          let mainClass = {};
          mainClass["@id"] = this.schema[i]["name"];
          mainClass["@type"] = "rdfs:Class";
          mainClass["rdfs:comment"] = this.schema[i]["description"];
          mainClass["rdfs:label"] = this.schema[i]["label"];
          mainClass["rdfs:subClassOf"] = { "@id": this.startingPoint };
          mainClass["$validation"] = {};
          // Add schema.org version
          if (this.schema_version) {
            mainClass["schemaVersion"] = [
              `https://schema.org/docs/releases.html#v${this.schema_version}`,
            ];
          }
          //Add New Class
          this.finalschema["@graph"].push(mainClass);
          //Handle main Class Properties
          if (this.schema[i]?.properties) {
            for (var x = 0; x < this.schema[i].properties.length; x++) {
              let mainProp = {};
              mainProp["@id"] = this.schema[i].properties[x]["name"];
              mainProp["@type"] = "rdf:Property";
              mainProp["rdfs:comment"] =
                this.schema[i].properties[x]["description"];
              mainProp["rdfs:label"] = this.schema[i].properties[x]["label"];
              mainProp["schema:domainIncludes"] = {
                "@id": this.schema[i]["name"],
              };
              //Ranges of Property
              if (this.schema[i].properties[x]["range"]) {
                let ranges = this.schema[i].properties[x]["range"].split(",");
              ...
              if (this.schema[i].properties[x].selected) {
                ...
                if (
                  this.previousPreviewProps.hasOwnProperty(
                    this.schema[i].properties[x].label
                  )
                ) {
              ...
              if (this.schema[i].properties[x].isRequired) {
                let p = this.schema[i].properties[x].label;
                if (!this.validation["required"].includes(p)) {
                  this.validation["required"].push(p);
                }
              }
              if (this.schema[i].properties[x].isRecommended) {
                let p = this.schema[i].properties[x].label;
                if (!this.validation["recommended"].includes(p)) {
                  this.validation["recommended"].push(p);
                }
              }
              if (this.schema[i].properties[x].isOptional) {
                let p = this.schema[i].properties[x].label;
                if (!this.validation["optional"].includes(p)) {
                  this.validation["optional"].push(p);
                }
              }
              //Add New Prop
              this.finalschema["@graph"].push(mainProp);
            }
          }
        } else {
          ...
          if (Object.hasOwnProperty.call(this.schema[i], "properties")) {
            let myLabel = "";
            for (let y = 0; y < this.schema[i].properties.length; y++) {
              if (this.schema[i].properties[y].selected) {
                let propValue = {};
                let prop_label = this.schema[i].properties[y]["label"];
                if (this.previousPreviewProps.hasOwnProperty(prop_label)) {
                  // keep existing validation if any
                  propValue = Object.assign(
                    {},
                    this.previousPreviewProps[prop_label]
                  );
                } else {
                  //for first time add description
                  propValue["description"] =
                    this.schema[i].properties[y].description;
                }
                myLabel = this.schema[i].properties[y].label;
                this.validation["properties"][myLabel] = propValue;
              }
              if (this.schema[i].properties[y].isRequired) {
                if (!this.validation["required"].includes(myLabel)) {
                  this.validation["required"].push(myLabel);
                }
              }
              if (this.schema[i].properties[y].isRecommended) {
                if (!this.validation["recommended"].includes(myLabel)) {
                  this.validation["recommended"].push(myLabel);
                }
              }
              if (this.schema[i].properties[y].isOptional) {
                if (!this.validation["optional"].includes(myLabel)) {
                  this.validation["optional"].push(myLabel);
                }
              }
              //pre populate validation from inherited if available
              if (
                Object.hasOwnProperty.call(
                  this.top_class_validation,
                  "properties"
                )
              ) {
                // console.log('pre-populating validation... ')
                for (const propName in this.top_class_validation.properties) {
                  if (
                    Object.hasOwnProperty.call(
                      this.validation["properties"],
                      propName
                    ...
                    // console.log('validation has ' + propName, Object.keys(this.validation['properties'][propName]).length)
                    if (
                      Object.keys(this.validation["properties"][propName])
                        .length == 1
                    ) {
                      //begin to add existing validation if validation only has default desc field.
                      for (const key in this.top_class_validation.properties[
                        propName
                      ]) {
                        if (key !== "description") {
                          // console.log('adding existing '+ key +' to ' + propName)
                          this.validation["properties"][propName][key] =
                            this.top_class_validation.properties[propName][key];
                        }
                      }
                    }
                  }
              ...
              for (const key in this.validation.properties) {
                let ref = addRefsMentioned(this.validation.properties[key]);
                if (ref) {
              ...
              if ([...defs_found].length) {
                let defs = [...defs_found];
                this.validation["definitions"] = {};
                defs.forEach((def) => {
                  this.definition_options.forEach((val) => {
                    if (val.title == def) {
                      // console.log('validation found',)
                      if (Object.hasOwnProperty.call(val, "validation")) {
                        this.validation.definitions[def] = val.validation;
                      }
                    }
                  });
                });
              ...
              if (this.finalschema["@graph"].length) {
                this.finalschema["@graph"][0]["$validation"] = this.validation;
              }
            }
          }
        }
      ...
      if (Object.hasOwnProperty.call(state?.validation, "properties")) {
        for (let key in this.validation["properties"]) {
          let keys = Object.keys(this.validation["properties"][key]).length;
          if (keys <= 1) {
            incomplete += 1;
          }
        }
        if (incomplete && !this.removeValidation) {
          new Notify({
            status: "warning",
            title: "Editor",
      ...
      // REMOVE VALIDATION
      if (this.removeValidation) {
        this.finalschema["@graph"].forEach((cls) => {
      ...
      if (JSON.stringify(this.finalschema).includes("bioschemas")) {
        this.finalschema["@context"]["bioschemas"] =
