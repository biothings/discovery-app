...
def monthly_routine():
    """Monthly routine for schema.org updates"""
    logger = logging.getLogger("monthly_routine")

    # Create a lock can only be acquired by one process.
    # Make sure only one process should perform monthly routines
    _lock = FileLock(".lock", timeout=0)
    ...
    try:
        ...
        if lock_acquired:
            ...
            with open(".lock", "w") as lock_file:
                lock_file.write(str(current_pid))
            logger.info(f"Monthly schedule lock acquired successfully. PID {current_pid} written to lock file.")
            logger.info("monthly_schemaorg_update()")
            monthly_schemaorg_update()
            _lock.release()
            logger.info("Monthly schedule lock released successfully.")
    except Timeout:
        existing_pid = get_lock_pid()
        if existing_pid:
            logger.warning(f"Monthly schedule lock acquired by another process (PID: {existing_pid}). No need to run it in this process.")
        else:
            logger.warning("Monthly schedule lock acquired by another process. No need to run it in this process.")
    except Exception as e:
        logger.error(f"An error occurred during the monthly routine: {e}")
        logger.error("Stack trace:", exc_info=True)
    finally:
        if lock_acquired:
            _lock.release()
...
def run_monthly_routine():
    thread = Thread(target=monthly_routine, daemon=True)
    thread.start()
...
if __name__ == "__main__":
    ...
    if not options.debug and options.prod:
        ...
        crontab("0 0 1 * *", func=run_monthly_routine, start=True)  # run monthly on the 1st at mid-night
